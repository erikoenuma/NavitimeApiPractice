    ...
    func passGeometry(startAddress: String, goalAddress: String){
        ...
        getGeometry(address: startAddress) { [self] (geometry) in
            if let geometry = geometry{
                ...
                navitimeAPI.setStartGeometry(lat: results.geometry.location.lat, lng: results.geometry.location.lng)
            }
        }
        ...
        getGeometry(address: goalAddress) { [self] (geometry) in
            if let geometry = geometry{
                guard let results = geometry.results.first else {return}
                navitimeAPI.setGoalGeometry(lat: results.geometry.location.lat, lng: results.geometry.location.lng)
            }
        }
...
//出発地・到着地の緯度経度、地点名を取得
    private func getGeometry(address: String, completion:@escaping((Geometry?) -> Void)){
